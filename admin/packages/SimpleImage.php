...
class SimpleImage {
	...
	function resize($width, $height) {
	  	$original_aspect = imagesx($this->image) / imagesy($this->image);
		$thumb_aspect = $width / $height;
		if ( $original_aspect >= $thumb_aspect )
		{
		   // If image is wider than thumbnail (in aspect ratio sense)
		   $new_height = $height;
		   $new_width = imagesx($this->image) / (imagesy($this->image) / $height);
		}
		else
		{
		   // If the thumbnail is wider than the image
		   $new_width = $width;
		   $new_height = imagesy($this->image) / (imagesx($this->image) / $width);
		}
